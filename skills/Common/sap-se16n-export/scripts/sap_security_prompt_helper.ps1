...
public static class SapSe16nWin32 {
...
function Get-WindowTextValue {
    ...
    [void][SapSe16nWin32]::GetWindowText($Handle, $buffer, $buffer.Capacity)
    $buffer.ToString()
}
...
function Get-ClassNameValue {
    ...
    [void][SapSe16nWin32]::GetClassName($Handle, $buffer, $buffer.Capacity)
    $buffer.ToString()
}
...
function Get-ProcessNameForWindow {
    param([IntPtr]$Handle)
    [uint32]$processId = 0
    [void][SapSe16nWin32]::GetWindowThreadProcessId($Handle, [ref]$processId)
    if ($processId -eq 0) {
        return ""
    }
    try {
...
function Get-ChildWindows {
    ...
    $callback = [SapSe16nWin32+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        [void]$children.Add($hWnd)
        return $true
    }
    [void][SapSe16nWin32]::EnumChildWindows($Parent, $callback, [IntPtr]::Zero)
    $children
}
...
function Confirm-KnownSapSecurityPrompt {
    ...
    if (-not [SapSe16nWin32]::IsWindowVisible($WindowHandle)) {
        return $false
    }
    ...
    foreach ($child in $children) {
        ...
        $controlId = [SapSe16nWin32]::GetDlgCtrlID($child)
    ...
    [void][SapSe16nWin32]::SendMessage($okButton, $ButtonClickMessage, [IntPtr]::Zero, [IntPtr]::Zero)
    return $true
}
...
while ((Get-Date) -lt $deadline) {
    ...
    [void][SapSe16nWin32]::EnumWindows($callback, [IntPtr]::Zero)
